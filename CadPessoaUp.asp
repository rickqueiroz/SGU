...
IF REQUEST("OPERACAO") = 1 THEN 'CADASTRAR
	call abreConexao
 		sql="INSERT INTO GU_CadPessoasUp(CPF, Nome, idPerfil, CodPrograma, status, DataCadUsuario, cpfCad) VALUES('"&replace(replace(replace(request.form("txtCPF"),".",""),".", ""),"-","")&"', '"&rtrim(request.form("txtNome"))&"','"&request.form("perfil")&"', '"&request.form("programas")&"' , 1 , getdate(),'"&request.form("cpf")&"')"
	conn.execute(sql)
	
	call fechaConexao 
	response.Redirect("CadPessoaUp.asp?resposta=1")
...
ELSEIF REQUEST("Operacao") = 3 THEN 'ALTERAR
	call abreConexao
	sql = "UPDATE GU_CadPessoasUp SET Nome = '"&rtrim(request.Form("txtNome"))&"', idPerfil = '"&request.form("perfil")&"', CodPrograma = '"&request.Form("programas")&"', status = '"&request.Form("status")&"' WHERE CPF = '"&replace(replace(replace(request.form("txtCPF"),".",""),".",""),"-","")&"'"
	conn.execute(sql)
	'resposta = 2
	call fechaConexao
	response.Redirect("CadPessoaUp.asp?resposta=2")
END IF
...
<body>
<%if request("resposta") = 1 or request("resposta") = 2 then%>
<script>
  var resp = <%=request("resposta")%>
  mensagem(resp)
</script>
<%end if%>
<div class="col-lg-10 mt-4">
    <div class="card mb-6">
      <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
        <h5 class="m-0 font-weight-bold text-primary">Cadastrar Usuários</h5>
      </div>
      <div class="card-body">
        ...
        <div class="col-4">
          ...
          <input type="text" class="form-control col-form-sm" onKeyPress="MascaraCPF(txtCPF)"  name="txtCPF" id="txtCPF"  value="<%=CPF%>" maxlength="14" onChange="MascaraCPF(txtCPF);verificar_cadastro();"<%IF REQUEST ("Operacao") = 2 AND Existe = 1 THEN%> readonly<%END IF%>/>
        </div>
            <div class="col">
...
<%
   call abreConexao
   sql = "SELECT GU_CadPessoasUp.CPF, GU_CadPessoasUp.Nome, GU_Perfil.Perfil, GU_Programas.Programas, convert(DateTime,DataCadUsuario,103) as DataCadUsuario,GU_CadPessoasUp.status AS statusUsuario FROM GU_CadPessoasUp INNER JOIN GU_Perfil ON GU_Perfil.idPerfil = GU_CadPessoasUp.idPerfil INNER JOIN GU_Programas ON GU_Programas.id = GU_CadPessoasUp.CodPrograma ORDER BY Nome;"
   set rs = conn.execute(sql)
%>
<div class="col-lg-12 mt-4">
    ...
    <div class="card mb-4">
        <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
            <h5 class="m-0 font-weight-bold text-primary">Lista de Usu&aacute;rios</h5>
        </div>
